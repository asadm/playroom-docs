...
If you are competent with chrome dev tools or the inspect element menu, you can mess around with playroom in the console by doing things like `Playroom.me()`, etc. to figure out what the Godot equivalent commands are for things like the React or Unity APIs, if you are coming from one of those. 
...
### How do I get a list of player states which I can use to set the states of players?
...
To get both the player objects and its states and do something to them for each player you could do something similar to the following:
```gdscript
#In process()...
...
```
Modify the code to see what fits you game. You can do a lot more than just positions, but it can make your functions really long.
