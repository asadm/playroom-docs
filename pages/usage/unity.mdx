...
#### Video Tutorial
<br />
<iframe style={{width: '100%'}} height="400" src="https://www.youtube.com/embed/1LXWZmJxlKA?si=nKZOaoiEj_WwXt6X" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; muted" allowfullscreen autoPlay muted></iframe>
...
To switch over to Browser mock mode, select the **"Browser Bridge (live)"** option from the "Mock mode" dropdown in the [Playroom Dev Manager](#playroom-dev-manager).
...
Once downloaded, place the chromedriver somewhere accessible. If working in teams, we **don't recommend** pushing the chromedriver to version control, as different developers in your team may have different versions of browsers. It's better if everyone downloads a version of their own.

Now, select **"UnityBrowserBridge"** from the project heirarchy.

<br />
<Image src="/images/unitybrowserbridge.png" width={800} height={700} />

This will open the "Unity Browser Bridge (Script)" panel on the right.

Here, update the path in the **"Web Driver Directory"** to wherever you put the chromedriver (as shown below).

<br />
<Image src="/images/playroomkit-for-unity-chromedriver.png" width={800} height={700} />

If you're using a *nix-based system like macOS or Linux, you should also update "Include Java Script Files" -> Element 0 with the correct path according to your system.

<Callout type='info' emoji="ℹ️">
    There are different ways to write paths based on OS.
    - for Windows, it's backslash `\`
    - for macOS, Linux (and more *nix based systems) it's forward slash `/`

    i.e. for Windows, path would be `Playroomkit\modules\PlayroomMockBridge.js`.
    While for macOS, path would be `Playroomkit/modules/PlayroomMockBridge.js`.
</Callout>
