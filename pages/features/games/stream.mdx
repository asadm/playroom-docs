...
When you pass `streamMode: true` to Playroom Kit's `insertCoin`. Playroom Kit automatically detects if it's running on a larger screen or a phone screen. If it's running on a larger screen, it will show the instructions to join the room. If it's running on a phone screen, it will let player pick their name and color.
