...
Alongside realtime games, Playroom Kit is great for turn-based and asynchronous games. These games are non-realtime and usually one player initiates a challenge and then sends it to another player. The other player can then play the game at their own pace and send their move back to the first player.

Turn-based API is built on top of the [Persistence API](/features/apps/persistence).
...
- Game can decide who's turn it is by checking the last element of the `getAllTurns` array.
...
- Players who have completed the lap can view the leaderboard by calling `getAllTurns`.
