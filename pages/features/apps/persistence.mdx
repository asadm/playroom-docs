...
# Persistent Storage in Playroom Kit

Playroom Kit [State](/features/apps/state) is cleared when all players leave the room. For some types of games, you may want to persist some data in your room that can be retrieved later when the player comes back to the same room. 

Playroom Kit provides a simple persistent storage API. The data is stored in the room until it is explicitly deleted or the expiry time is reached.
