...
In any of the scripts where you want to initialize and use Playroom Kit, add the following code at the top of the file:

```javascript
const { onPlayerJoin, insertCoin, isHost, myPlayer, getState, setState, Joystick } = Playroom;
```

### 3. Initialize Playroom Kit

In the same script, call the `insertCoin()` method likely somewhere inside your `prototype.initialize` to start Playroom Kit on page load.
...
GameManager.prototype.initialize = async function () {
  await insertCoin(); // Show Playroom Kit lobby screen and wait for players to join
  // ... your game code
}
```

### 4. Follow rest of the [Multiplayer Steps](/multiplayer) to build your game in PlayCanvas.
The rest of the steps are the same as for any other web game. You can also use all of APIs including [On-screen Joystick](/features/games/joystick), [Gamepads](/features/games/gamepads), [Stream mode](/features/games/stream), etc.
