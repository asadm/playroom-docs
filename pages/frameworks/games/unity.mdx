...
Initialize a Playroom Kit instance. We recommend initializing it in a Singleton and reusing it across your game.

Then, in your game's `Start` method, call [`InsertCoin`](/api-reference/js/insertCoin):

```csharp
using Playroom;

// ...
private Playroom Kit _playroomKit = new();
// ...
...
In your game's `Start` method, call [`Playroom Kit.InsertCoin`](/api-reference/unity#insertcoininitoptions-callback):
...
Now you can use Playroom Kit throughout your game!

### 3. Manage game state

Use [`Playroom Kit.SetState`](/api-reference/unity#setstatestring-key-t-value-bool-reliable--false-void) and [`Playroom Kit.GetState`](/api-reference/unity#getstatetstring-key-t) to handle game state. These automatically sync with other players.

See the [API reference](/api-reference/unity) for more information.
...
Do note that **Playroom Kit for Unity** only works with WebGL builds.

## Updating from a previous version

Update Playroom Kit for Unity to get the latest features and fixes. We recommend doing the following:
...
Playroom Kit for Unity ships with some helpful features to make multiplayer game dev easier.

### Playroom Dev Manager

The Playroom Kit for Unity plugin includes a "Playroom Dev Manager" panel for multi-instance testing and ease-of-use, which controls two features:
...
Playroom Kit for Unity, when installed in your project, comes with a prefab called **PlayroomMockManager**. This is available inside the **Playroom Kit/Prefabs** folder.
...
Still thinking about which mock mode to use? Check out [this FAQ answer](/frameworks/games/unity#which-mock-mode-should-i-use) covering different scenarios and which mock mode goes best for it.
...
1. Ensure the latest version of the [Playroom Kit plugin](/frameworks/games/unity#1-install-the-sdk) is installed.
...
Once built, this demo works on both WebGL in the browser, and in [Discord Activities](/features/integrations/discord).

Code for this example can be found [here](https://github.com/asadm/playroom-unity/tree/main/Assets/Playroom%20Kit/Examples/discord-activity).
...
<details>
  ...
  You can manage a list of players in Unity to track their state and perform actions on them simultaneously. Here’s how to use [OnPlayerJoin](https://docs.joinplayroom.com/api-reference/unity#onplayerjoincallback) and [Player.OnQuit](https://docs.joinplayroom.com/api-reference/unity#onquitcallback):
  ...
  This is similar to the Playroom Kit JavaScript SDK's [usePlayersList hook](https://docs.joinplayroom.com/api-reference/react#useplayerslisttriggeronplayerstatechange-boolean).
