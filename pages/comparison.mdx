...
**"Unity" stacks:** These are popular multiplayer stacks that has been around for a long time. A great choice for games that are complex and are made in Unity. Photon/Fishnet are a bit low-level than Playroom and still require you to write network code. You also need to manage rooms and lobbies yourself. 
...
**Hathora (and Hathora Cloud)** is a newer multiplayer stack that hosts your game server; running in a container, on a VM. Hathora only manages "hosting" your server. You still need to write your own server code, websocket code, and manage rooms and lobbies yourself.
