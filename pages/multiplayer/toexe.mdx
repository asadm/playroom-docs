...
**If you compile with an alternate operating system (aside from your own) you may have to use admin privileges to compress the files to a zip. On windows, even admin privileges will not work. Boot up the command line and use the `cd` command to get to your output directory. Then type `tar.exe -a -c -f output-zip-name.zip your-folder-name`
