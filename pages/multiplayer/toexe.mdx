...
2. Run the `nativefier https://shooter-matchmaking.joinplayroom.com --platform windows` command, replacing `https://shooter-matchmaking.joinplayroom.com` with whatever your domain is (can also be a sub-directory such as `your-game.something/game.html`). Also, make sure to replace `--platform windows` with whatever platform you are building for like `windows`, `linux`, `mac`, `osx`, etc (see [this](https://github.com/nativefier/nativefier/blob/HEAD/API.md#platform) for more platforms and more platform info)**
