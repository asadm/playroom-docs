...
|--------------|---------|---------|-----------------------------------------------------------------------------|
| `gameId`    | string  | undefined | The ID of the game from the Playroom developer portal. |
...
| `discord`    | boolean  | false | Enable Discord mode. See [Discord Mode](/components/discord) for more information. |
...
Allows the current host to assign host privileges to another player in the room.

Accepts playerId as input, identifying the target player.
This function can only be executed by the host.
The transfer will only occur if the target player has not left the room.
...
If `reconnectGracePeriod` is configured, the player must wait for a 5-second cooldown period before they can rejoin the same room. This cooldown allows the player's state to be safely restored upon rejoining.

If `reconnectGracePeriod` is not set, no cooldown is applied, and the player can leave and rejoin the room immediately. In this case, please note that the player’s state will not be restored upon rejoin.
...
| `keyboard`   | boolean | `true` | A boolean value to enable W, A, S and D keys to control joystick. |
...
## `getDiscordSDK(): Promise<DiscordSDK>`
