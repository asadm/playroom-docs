...
Tell Playroom Kit to start! Playroom Kit will then handle room creation, players joining and also let players pick their names, colors and avatars. Once host taps "Launch", the promise resolves. At this point you can start your game.

```cs
// Show Playroom Kit UI, let it handle players joining etc and wait for host to tap "Launch"
Playroom Kit.InsertCoin(new Playroom Kit.InitOptions()
{
  maxPlayersPerRoom = 2,
...
|--------------|---------|---------|-----------------------------------------------------------------------------|
| `gameId`    | string  | undefined | The ID of the game from the Playroom Kit developer portal. |
| `streamMode` | boolean | false   | If  `true` , Playroom Kit will start in [ stream mode ]( /multiplayer/stream ). |
| `allowGamepads` | boolean | false   | If  `true` , Playroom Kit will let players play game using gamepads connected to the stream device itself. This requires `streamMode` to also be `true`.<br/><br/>The gamepads need to be connected to device where stream screen is running. No phones are required in this mode but are optionally allowed as an alternative controller if there aren't enough physical gamepads in the room. Players who join via phone in this mode see an on-screen [Joystick](/multiplayer/joystick). |
| `baseUrl`    | string  | *Current Page URL*      | The base URL used for generating room link that host shares with other players. |
| `skipLobby` | boolean | false | Skips the Playroom Kit's lobby screen altogether. This is useful if you want to design your own multiplayer lobby. |
...
| `reconnectGracePeriod` | int | 0 | If set, Playroom Kit will wait for the given **number of milliseconds** for the player to reconnect to the room after a disconnect. <br/><br/>If the player reconnects within the grace period, the player's state is restored. If the player does not reconnect within the grace period, the player is removed from the room and `onQuit` fires for the player. |
| `maxPlayersPerRoom` | int | undefined | If set, Playroom Kit will set a maximum limit for the **number of players** per room. <br/><br/>If the room is full and a new player attempts to join, the Playroom Kit will display a default modal with a message stating that the "room is full", and the `insertCoin` method will throw an error with a message code `ROOM_LIMIT_EXCEEDED`. <br/><br/>If the room is full, `skipLobby` is set to `true`, and a new player is attempting to join the room, Playroom Kit will skip the default modal but will throw an error.  |
| `avatars`    | Array&lt;string&gt; | *Default Avatars* | An array of URLs to images that players can pick as their avatar. This will override the default avatars system that Playroom Kit provides. |
...
| `discord`    | boolean  | false | Enable Discord mode. See [Discord Mode](/components/games/discord) for more information. |
...
```cs
string winnerId = Playroom Kit.GetState<string>('winner');
...
```cs
Playroom Kit.SetState('winner', 'player1');

// Support for built in data types such as Vectors
Playroom Kit.SetState('position', gameObject.transform.position);
```

## `OnPlayerJoin(callback)`

Register a callback that will be called when a new player joins the room. The callback will be called with the player's [`Playroom Kit.Player`](#playroomkitplayer) object.

If a new player joins after the game has started, the callback is called with all existing [`Playroom Kit.Player`](#playroomkitplayer) objects, exclusively for the new player.

```cs
Playroom Kit.OnPlayerJoin((player) => {
...
```cs
if (Playroom Kit.IsStreamScreen()) {
...
Returns the current player's [`Playroom Kit.Player`](#playroomkitplayer) object.

```cs
Playroom Kit.Player player = Playroom Kit.MyPlayer();
Debug.Log($"Hello {player.GetProfile().name}!");
```

## `GetPlayer(id)`

Returns a player's [`Playroom Kit.Player`](#playroomkitplayer) object by their ID.

```cs
Playroom Kit.Player player = Playroom Kit.GetPlayer("abcd1234");
Debug.Log($"Hello {player.GetProfile().name}!");
```
...
Playroom Kit.ResetStates(statesToExclude, () =>
{
...
A `Playroom Kit.Player` object represents a player in the room. In the Unity SDK, this is usually retrieved through `OnPlayerJoin`, `MyPlayer` or `GetPlayer`.

It has the following methods:

## `GetProfile(): Playroom Kit.Player.Profile`
...
```cs
Playroom Kit.RpcRegister("Shoot", CallBackFunction, "You shot a bullet!");
