...
We applied for Apple's Developer Labs to get access to Vision Pro at Cupertino. Luckily, we got selected for the first day! I was probably one of the earliest non-Apple devs to get my hands on Vision Pro.

My main goal was to try a few code demos, mostly explore WebXR on it, make a multiplayer demo using [Playroom](https://docs.joinplayroom.com) and try to benchmark the device for a bunch of AI use-cases that I am interested in (like inference time with Stable Diffusion running on-device). Unfortunately, I am not allowed to talk much about my experience. But it was positive overall and I had fun!
...
I added Playroom SDK which handles the multiplayer and gamepad API for me:
```js {6, 11 }
...
My game can now be played using game controllers. I just needed to read the gamepad state and update the player's plane position in my main game loop:
```js {6}
function render() {
